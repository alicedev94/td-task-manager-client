<script setup lang="ts">
  ...
  const { value: Departament } = useField<{ id: number; name: string }>(
    'Departament'
  );
  const roles = ref();
  ...
  const getDepartament = async () => {
    try {
      const response = await axios.get(
        `${dataBaseUrlDepartament}/getDepartament`
      );
    ...
    >
      ...
      <div class="card flex flex-wrap gap-4 w-full">
        <div class="flex flex-col gap-2 flex-1">
          ...
          >
            <label for="Rol"
              >Tipo de Rol<span class="text-red-500">*</span></label
            >
            <Select
              id="role"
              v-bind="field"
