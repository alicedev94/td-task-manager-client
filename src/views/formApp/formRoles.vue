<script setup lang="ts">
  ...
  const { value: Id_Permissions } = useField<{ id: number; name: string }>(
    'Id_Permissions'
  );
  ...
  const getPermissions = async () => {
    try {
      const response = await axios.get(
        `${dataBaseUrlPermissions}/getPermissions`
      );
  ...
  const onFormSubmit = (values: any) => {
    ...
    const ids: string = values.Id_Permissions.value
      .map((permission: Ids) => permission.id)
      .join(', ');
    ...
    >
      <div class="card flex flex-wrap gap-4 w-full">
        <div class="flex flex-col gap-2 flex-1">
          ...
          >
            <label for="Name"
              >Nombre del Rol<span class="text-red-500">*</span></label
            >
            <InputText
              v-bind="field"
              type="text"
        ...
        <div class="flex flex-col gap-2 flex-1">
          <Field
            name="Id_Permissions"
            rules="required|Id_Permissions"
            v-slot="{ field }"
            v-model="Id_Permissions"
          >
            <label for="Id_Permissions"
              >Permisos<span class="text-red-500">*</span></label
            >
            <MultiSelect
              v-bind="field"
              :options="permissions"
